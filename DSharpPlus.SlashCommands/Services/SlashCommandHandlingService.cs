...
namespace DSharpPlus.SlashCommands.Services {
    public class SlashCommandHandlingService {
        ...
        private async Task ExecuteInteraction(BaseDiscordClient discord, DiscordInteraction interact,
            DiscordSlashClient c, CancellationToken cancellationToken) {
            try {
        ...
        private void LoadCommandTree() {
            ...
            foreach (var cmd in commandMethods) {
                ...
                if ((attr = cmd.GetCustomAttribute<SlashSubcommandAttribute>(false)) is null) {
                ...
                else { //... if it is a subcommand, get the class that the subcommand is in...
                    ...
                    if (subGroupClass is null ||
                        (subGroupAttr =
                            subGroupClass.GetCustomAttribute<SlashSubcommandGroupAttribute>(false)) is null) {
                        // ... otherwise tell the user a subcommand needs to be in a subcommand group
                        throw new Exception(
                            "A Subcommand is required to be inside a class marked with a SubcommandGroup attribute");
                    }
                    ...
                    if (slashCmdClass is null || (slashAttr =
                        slashCmdClass.GetCustomAttribute<SlashCommandAttribute>(false)) is null) {
                        // ... otherwise tell the user a subcommand group needs to be in a slash command class
                        throw new Exception(
                            "A Subcommand Group is required to be a child of a class marked with a SlashCommand attribute");
                    }

                    //... if it is a slash command, get or add the SlashCommand for the command ...
                    if (!commands.ContainsKey(slashAttr.Name)) {
                        commands.Add(slashAttr.Name,
                            new SlashCommand(slashAttr.Name, Array.Empty<SlashSubcommandGroup>(), slashAttr.GuildId));
                    }
                    ...
                    if (commands.TryGetValue(slashAttr.Name, out var slashCommand)) {
                        ...
                        if (slashCommand.Subcommands is null)
                            throw new Exception(
                                "Can't add a subcommand to a Slash Command without subcommands.");
                        ...
                        if (!slashCommand.Subcommands.ContainsKey(subGroupAttr.Name))
                            slashCommand.Subcommands.Add(subGroupAttr.Name,
                                new SlashSubcommandGroup(subGroupAttr.Name,
                                    subGroupClass.GetCustomAttribute<DescriptionAttribute>()?.Description ??
                                    "n/a"));

                        if (slashCommand.Subcommands.TryGetValue(subGroupAttr.Name,
                            out var slashSubcommandGroup)) {
                        ...
                        else { //... otherwise tell the user no subcommand was found.
                            throw new Exception("Failed to get a subcommand grouping!");
                        }
                    }
                    else { // ... otherwise tell the user no slash command was found.
            ...
            foreach (var cmd in nonSubcommandCommands) {
                ...
                SlashCommandAttribute? attr = null;
                DynamicSlashCommandAttribute? dynamic = null;
                if ((attr = cmd.GetCustomAttribute<SlashCommandAttribute>(false)) is null &&
                    (dynamic = cmd.GetCustomAttribute<DynamicSlashCommandAttribute>(false)) is null) {
                    continue;
                }
                ...
                if (cmd.DeclaringType is null || !cmd.DeclaringType.IsSubclassOf(typeof(BaseSlashCommandModule))) {
                    throw new Exception("A SlashCommand method needs to be in a class.");
                }

                // here we are working with a normal slash command
                if (attr is not null) {
                    if (cmd.GetCustomAttribute<DynamicSlashCommandAttribute>(false) is not null) {
                        throw new Exception("A command can not be a dynamic command as well");
                    }
                ...
                else if (dynamic is not null) {
                    if (cmd.GetCustomAttribute<SlashCommandAttribute>(false) is not null) {
                        throw new Exception("A command can not be a normal slash command as well");
                    }

                    if (cmd.ReturnType != typeof(IDictionary<CommandInfo, Action<InteractionContext>>)) {
                        throw new Exception(
                            $"Function ${cmd.Name} does not have correct signature (should be IDictionary<string, Action<InteractionContext>>)");
                    }
                    ...
                    var result = (IDictionary<CommandInfo, Action<InteractionContext>>) (cmd.Invoke(instance, null) ??
                        new Dictionary<string, Action<InteractionContext>>());
                    foreach (var (info, action) in result) {
                        if (commands.ContainsKey(info.Name)) {
                            throw new Exception($"A command with the name {info.Name} already exsists.");
                        }

                        var guildId = info.GuildId != 0 ? info.GuildId : dynamic.GuildId;
                        commands.Add(info.Name, new SlashCommand(info.Name, new SlashSubcommand(
                                info.Name,
                                info.Description,
                                action.Method,
                                action
        ...
        private async Task BulkUpdateCommands() {
            Dictionary<ulong,List<DiscordApplicationCommand>> commandDictionary = new();
            foreach (SlashCommand c in Commands.Values) {
                var command = BuildApplicationCommand(c);
                var id = c.GuildId.GetValueOrDefault();
                if (!commandDictionary.ContainsKey(id)) {
                    commandDictionary.Add(id, new List<DiscordApplicationCommand>());
                }
                commandDictionary[id].Add(command);
            }

            foreach (var (id, list) in commandDictionary) {
                ...
                msg.RequestUri = id == 0 ? 
                    new Uri($"https://discord.com/api/applications/{BotId}/commands") : 
                    new Uri($"https://discord.com/api/v8/applications/{BotId}/guilds/{id}/commands");
                ...
                if (response.IsSuccessStatusCode) {
                    ...
                    foreach (DiscordApplicationCommand newCommand in commands) {
                        ...
                        if (newCommand is null || oldCommand is null) {
                            continue;
                        }
        ...
        private DiscordApplicationCommand BuildApplicationCommand(SlashCommand cmd) {
            ...
            if (cmd.Subcommands is not null) { // ... for every subcommand, add the option for it.
                foreach (var sub in cmd.Subcommands)
                    builder.AddOption(GetSubcommandOption(sub.Value));
            }
            else if (cmd.Command is not null) { // ... otherwise directly add the paramater options for this command ...
                var parameters = cmd.Command.ExecutionMethod.GetParameters();
                if (parameters.Length > 1) { // ... if there are any other paramaters besides the Interaction.
        ...
        private ApplicationCommandOptionBuilder GetSubcommandOption(SlashSubcommand cmd) {
            ...
            if (parameters.Length > 1) { // ... load the parmeter options in.
                builder.Options = GetCommandAttributeOptions(parameters[1..]);
            }
        ...
        private List<ApplicationCommandOptionBuilder> GetCommandAttributeOptions(ParameterInfo[] parameters) {
            ...
            foreach (var param in parameters) { // ... propograte the inital command options ...
                var b = new ApplicationCommandOptionBuilder()
                    .WithName(param.Name ?? "noname") // ... with a name ...
                    .WithDescription(param.GetCustomAttribute<DescriptionAttribute>()?.Description ??
                                     "n/a") // ... a description ...
                    .IsRequired(!param.HasDefaultValue) // ... if it is required or not ...
                    .IsDefault(param
                        .GetCustomAttribute<DefaultParameterAttribute>() is not null); // ... if it is the default ...
                // ... then see if it is an enum ...
                if (param.ParameterType.IsEnum) { //... and load it in as an int with choices ...
                    b.WithType(ApplicationCommandOptionType.Integer)
                        .WithChoices(param.ParameterType);
                }
                else { // ... or as a regualr parameter ...
